...
class Agent():
    ...
    def __init__(self, state_size, action_size, seed , augment_frames = 1):
        ...
        """
        self.state_size = state_size * augment_frames
    ...
    def act(self, state, eps=0. , debug = False):
        ...
        if (len(self.memory.memory) >= 2 and (self.memory.memory[-1].done or self.memory.memory[-2].done) == False):
            ...
            state = torch.from_numpy(np.array([self.memory.memory[-2].state ,self.memory.memory[-1].state , state]).astype(np.float32)).float().view(1,-1).to(device)
            if debug:
...
class ReplayBuffer:
    ...
    def sample_idx(self , peek_a_boo = False):
        ...
        states = torch.from_numpy(np.stack([[self.memory[e[0]-2].state,self.memory[e[0]-1].state ,e[1].state] for e in index_value if e is not None and self.memory[e[0]-2].done != True and self.memory[e[0]-1].done != True and e[0]-2 > 0 and e[0] < len(self.memory)-1] , axis = 0 )).float().to(device)
        actions = torch.from_numpy(np.vstack([e[1].action for e in index_value if e is not None and self.memory[e[0]-2].done != True and self.memory[e[0]-1].done != True and e[0]-2 > 0 and e[0] < len(self.memory)-1])).long().to(device)
        rewards = torch.from_numpy(np.vstack([e[1].reward for e in index_value if e is not None and self.memory[e[0]-2].done != True and self.memory[e[0]-1].done != True and e[0]-2 > 0 and e[0] < len(self.memory)-1])).float().to(device)
        next_states = torch.from_numpy(np.stack([[self.memory[e[0]-2].next_state, self.memory[e[0]-1].next_state ,e[1].next_state] for e in index_value if e is not None and self.memory[e[0]-2].done != True and self.memory[e[0]-1].done != True and e[0]-2 > 0 and e[0] < len(self.memory)-1] , axis = 0 )).float().to(device)
        dones = torch.from_numpy(np.vstack([e[1].done for e in index_value if e is not None and self.memory[e[0]-2].done != True and self.memory[e[0]-1].done != True and e[0]-2 > 0 and e[0] < len(self.memory)-1]).astype(np.uint8)).float().to(device)
